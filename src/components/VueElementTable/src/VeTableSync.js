        ...
        handleHttpRequest({ pageSize, currentPage } = {}) {
            ...
            Promise.resolve(this._httpRequest(payload))
                .then(val => {
                    ...
                    const isCurrent =
                        payload.pageSize === this.innerPageSize &&
                        payload.currentPage === this.innerCurrentPage;
                    if (!isCurrent) return;
                    if (isArray(val)) {
        ...
        total: {
            ...
            handler(val) {
                ...
                if (ceilTotalPage >= this.innerCurrentPage) {
                    ...
                    console.warn(
                        `[ve-table]: total ${val} less than ${curTotal} (pageSize*currentPage, ${
                            this.innerPageSize
                        }*${this.innerCurrentPage}), set total to ${curTotal}`
                    );
